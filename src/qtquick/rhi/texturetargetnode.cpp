  ...
  , m_projectionMatrix(projectionMatrix)
{
  ...
  if (!m_clippingVertexBuffer) {
    // some large buffer we have here
    m_clippingVertexBuffer = rhi->newBuffer(QRhiBuffer::Dynamic, QRhiBuffer::VertexBuffer, m_maximumClippingVerticies * sizeof(QVector2D));
...
void TextureTargetNode::prepareRender(QRhiCommandBuffer *cb)
{
  ...
  if (m_oldBufferSize > m_geometryData.size()) {
    m_resourceUpdates->updateDynamicBuffer(
      m_vertexBuffer, 0, qMin((unsigned long long)m_clearData.size(), m_maximumVerticies * sizeof(QVector2D)), m_clearData.constData());
    ...
    m_resourceUpdates->updateDynamicBuffer(m_clippingVertexBuffer, 0,
                                           qMin((unsigned long long)m_clippingData.size(), m_maximumClippingVerticies * sizeof(QVector2D)),
...
void TextureTargetNode::renderBlend(QRhiCommandBuffer *cb)
{
  ...
  if (m_shaderBlending) {
    ...
    if (!m_blendSampler) {
      m_blendSampler =
        rhi->newSampler(QRhiSampler::Nearest, QRhiSampler::Nearest, QRhiSampler::None, QRhiSampler::ClampToEdge, QRhiSampler::ClampToEdge);
